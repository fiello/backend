...
{
	using namespace std;
	boost::mutex g_fileAccess;

	CErrorLevel::etLogLevel CLogger::m_eLogLevel = CErrorLevel::Error; // default log level
	...
	////////////////////////////////////////////////////////////////////
	/*
	 * Set static variable with the full path to the log file. Current design is to put
	 * log file next to the binary file
	 * @param strLogDir - in, reference to the string wih directory path
	 */
	...
	void CLogger::Out(const char *szFunctionName, CErrorLevel::etLogLevel eErrorLevel, const char *szMsg)
	{
		try
		{
			...
			if (m_strLogFilePath.empty() || eErrorLevel == CErrorLevel::None)
				return;

			boost::lock_guard<boost::mutex> lock(g_fileAccess);
